...
bool DbNet::initModel(AAssetManager *mgr) {
    int ret_param = net.load_param(mgr, "ocr_models_ncnn/dbnet_op.param");
    int ret_bin = net.load_model(mgr, "ocr_models_ncnn/dbnet_op.bin");
    if (ret_param != 0 || ret_bin != 0) {
        LOGE("#dbnet: %d  %d", ret_param, ret_bin);
        return false;
    }
    return true;
}
