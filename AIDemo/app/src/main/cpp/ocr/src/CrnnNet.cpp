...
char *readKeysFromAssets(AAssetManager *mgr) {
    ...
    char *buffer;
    /*获取文件名并打开*/
    AAsset *asset = AAssetManager_open(mgr, "ocr_keys.txt", AASSET_MODE_UNKNOWN);
    if (asset == NULL) {
        LOGE("ocr_keys.txt %s", "asset==NULL");
        return NULL;
    }
    /*获取文件大小*/
...
bool CrnnNet::initModel(AAssetManager *mgr) {
    int ret_param = net.load_param(mgr, "ocr_models_ncnn/crnn_lite_op.param");
    int ret_bin = net.load_model(mgr, "ocr_models_ncnn/crnn_lite_op.bin");
    if (ret_param != 0 || ret_bin != 0) {
        LOGE("#CrnnNet %d  %d", ret_param, ret_bin);
        return false;
    }
